...
class BayesianMutationDecision:
    ...
    def compute_mutation_gain(self, evidence: MutationEvidence) -> float:
        ...
        Returns:
            综合收益值
        """
        gain = (self.alpha * evidence.mutual_info_gain +
                self.beta * evidence.cond_mutual_info_gain +
                self.gamma * evidence.uncertainty_reduction -
                self.delta * evidence.transfer_cost)
    ...
    def update_prior_from_history(self, prior: MutationPrior) -> MutationPrior:
        ...
        if historical_gains:
            # 贝叶斯更新：结合先验和历史数据
            prior_precision = 1.0 / (prior.std_gain ** 2)
            data_precision = len(historical_gains) / (np.var(historical_gains) + 1e-8)
            
            # 更新均值（精度加权平均）
            updated_mean = (prior_precision * prior.mean_gain + 
                          data_precision * np.mean(historical_gains)) / (prior_precision + data_precision)
            
            # 更新标准差
            updated_std = np.sqrt(1.0 / (prior_precision + data_precision))
            ...
            return MutationPrior(
                ...
                std_gain=max(updated_std, 0.01),  # 避免过度自信
                failure_prob=max(updated_failure_prob, 0.1),
    ...
    def compute_posterior_distribution(self,
        ...
        # 证据的可靠性（基于互信息的置信度）
        evidence_reliability = min(1.0, evidence.mutual_info_gain / 0.1 + \
                                 evidence.uncertainty_reduction / 0.05)
        
        # 贝叶斯更新：结合先验和证据
        prior_precision = 1.0 / (prior.std_gain ** 2)
        evidence_precision = evidence_reliability / 0.01  # 证据精度
        
        # 后验均值
        posterior_mean = (prior_precision * prior.mean_gain + 
                         evidence_precision * evidence_gain) / (prior_precision + evidence_precision)
        
        # 后验标准差
        posterior_std = np.sqrt(1.0 / (prior_precision + evidence_precision))
    ...
    def compute_expected_utility(self,
        ...
        utility_mean = 1.0 - np.exp(-self.risk_aversion * posterior_mean)
        utility_curvature = -self.risk_aversion**2 * np.exp(-self.risk_aversion * posterior_mean)
    ...
    def make_decision(self,
                     ...
                     utility_threshold: float = 0.0) -> MutationDecision:
        """
        基于贝叶斯框架做出变异决策
        ...
        # 做出决策
        should_mutate = expected_utility > utility_threshold
    ...
    def record_mutation_outcome(self,
        ...
        if len(self.mutation_history) > 100:
            self.mutation_history = self.mutation_history[-50:]  # 保留最近50次
    ...
    def adjust_risk_aversion(self, training_stage: str):
        ...
        if training_stage == 'early':
            self.risk_aversion = 1.0  # 更激进
        elif training_stage == 'middle':
            self.risk_aversion = 2.0  # 中等
        else:  # late
            self.risk_aversion = 3.0  # 更保守